...
struct XcodeSchemeLoaderTests {
    ...
    func loadSchemesWithProjectReference() async throws {
        ...
        let projectLocation = XcodeProjectLocation.explicitWorkspace(workspaceDir)

        // Test loading all schemes
        let allSchemes = try await schemeLoader.loadSchemes(from: projectLocation, filteredBy: nil)
        #expect(allSchemes.count == 2) // App.xcscheme and Tests.xcscheme

        // Test loading with specific scheme filter
        let projectReference = XcodeProjectReference(scheme: "App")
        let filteredSchemes = try await schemeLoader.loadSchemes(from: projectLocation, filteredBy: projectReference)
    ...
    func loadSchemesThrowsForInvalidSchemeFilter() async throws {
        ...
        let projectLocation = XcodeProjectLocation.explicitWorkspace(workspaceDir)
        let invalidReference = XcodeProjectReference(scheme: "NonExistentScheme")

        await #expect(throws: XcodeProjectError.self) {
            try await schemeLoader.loadSchemes(from: projectLocation, filteredBy: invalidReference)
        }
    }
